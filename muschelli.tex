...
\title{Gathering Bibliometric Information from the Scopus API using the
...
\texttt{R} package \texttt{rscopus}, interacing with the Elsevier Scopus
API. We demonstrate how to manipulate the output from the API into
organized data that can be analyzed. We present options on how to obtain
the number of citations from an author and use that to calculate
citation metrics with the output. We also show how to download
additional information from an article such as the figures and
...
The \pkg{rscopus} package allows users to interface with Scopus APIs and
...
There are other helpful functions such as \texttt{have\_api\_key}, which
...
\texttt{content} from \CRANpkg{httr} \citep{httr}, which calls
\texttt{fromJSON} from \CRANpkg{jsonlite} \citep{jsonlite}). This column
...
co-authored publications. We will use \CRANpkg{dplyr} \citep{dplyr} for
data manipulation:
...
\pkg{httr}, there are elements of the list named \texttt{entries} or
...
\texttt{response} (from the \pkg{httr} package), and \texttt{content},
...
many respects, but parts can be combined using \CRANpkg{purrr}
\citep{purrr}:
...
list in the \CRANpkg{stopwords} package \citep{stopwords}:
