...
data that can be analyzed. We present options on how to calculate the
number of citations from an author, as well as calculating citation
metrics with the output.
}
...
Although \pkg{rscopus} does not provide these tools specifically, it can
...
interface. For example, the \pkg{bibliometrix} \citep{bibliometrix}
...
functionality from \pkg{rscopus}. The \pkg{bibliometrix} package also
...
GitHub package \pkg{rwos} (\url{https://github.com/juba/rwos}), but is
...
these sources is more curated. Other packages such as \pkg{scholar}
\citep{scholar} and \pkg{gcite} \citep{gcite} can provide interfaces to
the Google Scholar citation information. Using these in combination with
\pkg{rscopus} can more likely guarantee complete information. Also,
...
\emph{h}-index \citep{hirsch2005index}. We will use \pkg{dplyr}
...
Using \pkg{ggplot2} \citep{ggplot2}, we can also visually show the
...
\texttt{response} (from the \pkg{httr} package \citep{httr}), and
...
done by \pkg{tidyr} \citep{tidyr}. Here we use \pkg{dplyr} to arrange
...
\includegraphics{index_files/figure-latex/unnamed-chunk-24-1} \end{Schunk}
...
in the \pkg{stopwords} package \citep{stopwords}:
